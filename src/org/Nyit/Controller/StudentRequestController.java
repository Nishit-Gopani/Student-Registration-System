...
//					   String [] srt = request.getParameter("schedule_Id").trim().split(",");
//						int [] value = new  int [srt.length];
//						  for (int i = 0; i < srt.length; i++) {
//						  value[i] = Integer.parseInt(srt[i]);
//						  }
					   dao.seatCount(request.getParameter("schedule_Id"));
					   dao.updateStatus(studentId);
					   registrationRequestVO.setApproveDeclineBy(request.getSession().getAttribute("currentUser").toString());
